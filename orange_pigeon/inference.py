...
# ---------------------------------------------------------------------------
# Configuration (Exact Copy of Example Pattern)
# ---------------------------------------------------------------------------
...
async def main() -> None:
    # Exit check exactly like their example
    if not API_KEY:
        raise SystemExit("API_KEY (or HF_TOKEN) must be set to query the model.")

    # Client init using the global variables
    ...
    try:
        ...
        for step in range(1, 11):
            ...
            # Calling API exactly as per example style
